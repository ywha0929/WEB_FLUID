...
void main()
{
    ...
    lowp vec3 ep_blur_color = texture2D(ep_blur_img,  output_text_coord).rgb;
    ...
    if (face_num > 0)
    {
        ...
        skin_prob              = face_mask.b > 0.005 ? min(orig_skin_prob, face_mask.r) : (orig_skin_prob * face_mask.a);
    ...
    vec3 natural_smooth = mix(ep_blur_color, mean_color, variance);

    // blend
    mediump float final_blurred_ratio = clamp(skin_prob * variance_attenuation * dark_attenuation * smooth_intensity, 0.0, 1.0);
    
    mediump vec3  result_color        = mix(orig_color.rgb, natural_smooth, final_blurred_ratio);
